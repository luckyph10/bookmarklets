...
const duplicateCommentsInput=document.getElementById("dp-duplicate-comments");
...
const processState=()=>{
    ...
    const duplicateValue=duplicateCommentsInput.value;

    if(!duplicateValue){
        status.textContent="Select Duplicate Comments.";
...
const finish=eligibleToday=>{
    ...
    if(!currentName||!state||!duplicateComments)return;
...
const planTypes=[
...document.querySelectorAll('[id^="planType\\_"]')
]
